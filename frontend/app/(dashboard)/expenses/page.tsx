...
const emptyForm = { amount_cents: 0, description: '', vendor: '', date: '', category_name: '' }
...
export default function ExpensesPage() {
  ...
  const handleCreate = async () => {
    setSaving(true)
    try {
      await api.expenses.create({ ...form, amount_cents: Math.round(form.amount_cents * 100) })
      setModalOpen(false)
      setForm(emptyForm)
      load()
    } catch (e: any) {
              ...
              {data?.items.map((exp) => (
                <div key={exp.id} className="flex items-center justify-between py-3 border-b last:border-0">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{exp.vendor_source} &middot; {exp.date}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <Badge variant="danger">{fmtCents(exp.amount_cents)}</Badge>
                    <button onClick={() => handleDelete(exp.id)} className="text-xs text-red-600 hover:underline">Delete</button>
                  </div>
                </div>
              ))}
              {data?.items.length === 0 && <p className="text-center py-8 text-gray-500">No expenses found</p>}
            </div>
            {data && data.pages > 1 && (
